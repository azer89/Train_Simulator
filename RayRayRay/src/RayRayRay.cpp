...
bool RayRayRay::mousePressed(const OIS::MouseEvent& arg, OIS::MouseButtonID id)
{
	if(id == OIS::MB_Left)
	{
		...
		for(iter; iter != result.end(); iter++)
		{
			...
			if(iter->movable && iter->movable->getName().substr(0, 5) != "tile[" && iter->movable->getTypeFlags() != 0xFFFFFFFF)
			{
				Ogre::SceneNode* tempNode = iter->movable->getParentSceneNode();
				//char buf[255];
				//sprintf(buf, "%x", iter->movable->getTypeFlags());
				//std::cout << buf << " " << tempNode->getName() << "\n";
